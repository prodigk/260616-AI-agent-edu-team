...
`docs/AGENTS.md`를 기준으로 유지합니다.

채팅 기능은 대시보드의 핵심 경험이므로 별도 스펙으로 관리합니다.
채팅 UI, 담당 에이전트, 제품총괄 종합 답변, OpenAI/fallback, 스트리밍,
로그 저장 요구사항은 `docs/Chat Feature Spec.md`를 참고하세요.
...
담당하고, 우측 채팅에서는 다음 직무 에이전트가 분석 결과를 역할별로
설명합니다.
...
`제품총괄`은 다른 담당 답변을 종합·평가해 최종 의사결정 관점으로
정리합니다.
역할별 답변 지침은 `agent/` 폴더의 Markdown 파일에서 관리합니다.
